...
* @returns {any}
*/
export function next_turn(histories, text) {
    const ptr0 = passStringToWasm0(histories, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ret = wasm.next_turn(ptr0, len0, ptr1, len1);
    return takeObject(ret);
}
...
async function init(input) {
    if (typeof input === 'undefined') {
        input = new URL('shiritori_wasm_bg.wasm', import.meta.url);
    }
    const imports = {};
    imports.wbg = {};
